...
export async function ensureExternalItemSchema(): Promise<void> {
  ...
  await pool.query(`CREATE INDEX IF NOT EXISTS items_title_trgm_idx ON items USING gin (title gin_trgm_ops);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS items_description_trgm_idx ON items USING gin (description gin_trgm_ops);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS items_item_category_trgm_idx ON items USING gin (item_category gin_trgm_ops);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS items_color_trgm_idx ON items USING gin (color gin_trgm_ops);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS items_location_trgm_idx ON items USING gin (location gin_trgm_ops);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS items_address_trgm_idx ON items USING gin (address gin_trgm_ops);`);
  await pool.query(`CREATE INDEX IF NOT EXISTS items_place_name_trgm_idx ON items USING gin (place_name gin_trgm_ops);`);
}
