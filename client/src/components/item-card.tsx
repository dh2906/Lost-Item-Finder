...
import { MapPin, Calendar, Tag as TagIcon, Sparkles, ChevronDown, ChevronUp } from "lucide-react";
...
export function ItemCard({ item, score, reasoning, distanceText, className, variant = "default" }: ItemCardProps) {
  ...
  return (
    <Link href={`/item/${item.id}`} className={cn("group block h-full outline-none", className)}>
      <motion.div
        whileHover={{ y: -4 }}
