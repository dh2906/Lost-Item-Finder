...
## 언어 규칙
- 사용자에게 전달하는 답변, 작업 요약, 커밋 메시지, PR 제목, PR 본문은 모두 한국어로 작성한다.
- 코드 식별자, 명령어, 환경변수명, 로그 라벨처럼 원문 유지가 필요한 기술 용어만 영어를 허용한다.
- GitHub PR을 만들 때 `Summary`, `Verification` 같은 기본 영어 섹션명을 쓰지 말고 `작업 요약`, `검증`처럼 한국어 제목을 사용한다.
...
## 커밋 원칙 보강
- 서로 다른 의도의 변경은 최대한 작은 커밋으로 나누어 작성한다.
- 기능 변경, 버그 수정, 문서 수정, 검증용 설정 변경은 가능하면 별도 커밋으로 분리한다.
- 커밋 메시지와 PR 제목/본문은 사용자에게 전달되는 문서이므로 한국어로 작성한다.
